...
// createUpdateController takes an injected deps object `{ check, relaunch }` so a
// fake plugin can stand in without a Tauri host. `check` is the
// @tauri-apps/plugin-updater check(); `relaunch` is @tauri-apps/plugin-process
// relaunch(). The behaviour asserted (version surfaced, percent from
// contentLength/chunkLength, relaunch callable) is the contract.
...
describe("createUpdateController over a fake plugin", () => {
  ...
  // side-effect-contract: an absent contentLength must NOT produce NaN% - the
  // percent branch is skipped entirely, only the final 100 is reported
  it("should not report NaN percent if contentLength is absent", async () => {
    const controller = createUpdateController({
      check: () =>
        Promise.resolve({
          version: "v0.2.0",
    ...
    const reported = onProgress.mock.calls.map((call) => call[0]);
    expect(reported.every((pct) => !Number.isNaN(pct))).toBe(true);
