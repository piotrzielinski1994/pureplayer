...
// overrides) sets an inline CSS var (e.g. --primary) on el.style for each provided
// app token and CLEARS any app-token var not present in `overrides` (so a
// previously applied override is removed on a mode/colors change). The var name
// for a hyphenated token (`card-foreground`) is the dashed `--card-foreground`.
// PP model is FLATTENED: `overrides` is a flat token->oklch map. We assert via
// el.style.getPropertyValue(...) on a detached element.

const tokens = (partial: ThemeColorOverrides): ThemeColorOverrides => partial;
...
describe("applyThemeVars", () => {
  ...
  // spec §5.1 - side-effect-contract: a hyphenated token maps to the dashed var.
  ...
  // AC-008 / spec §5.5 - side-effect-contract: clearing on a colors change. A var
  ...
  // AC-007 / spec §5.5 - side-effect-contract: only the provided tokens are set;
  // the rest of the app-token vars stay clear (no stray vars leak in).
  ...
  // spec §5.5 - side-effect-contract: an empty overrides map leaves no stray vars.
  it("should leave no inline app-token vars if the overrides are empty", () => {
    const el = document.createElement("div");

    applyThemeVars(el, "light", tokens({}));

    expect(el.getAttribute("style")).toBeNull();
  });

  // spec §5.5 - side-effect-contract: switching the override from one token to
