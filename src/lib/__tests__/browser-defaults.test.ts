...
describe("isReservedBrowserShortcut", () => {
  ...
  reserved.forEach(([label, args]) => {
    // behavior: every browser-reserved combo is flagged for suppression
  ...
  kept.forEach(([label, args]) => {
    // behavior: text-editing and OS combos are NOT suppressed
    it(`should return false if the combo is ${label}`, () => {
      expect(isReservedBrowserShortcut(key(args))).toBe(false);
    });
  });

  // behavior: a bare reserved key (no modifier) is an app hotkey, not a browser default
...
describe("installBrowserDefaultGuards", () => {
  ...
  // side-effect-contract: a reserved browser shortcut has its default action blocked
  ...
  // side-effect-contract: a text-editing shortcut passes through untouched
