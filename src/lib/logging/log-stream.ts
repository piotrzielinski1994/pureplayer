...
// Port for the application log stream (F2). The native build subscribes to tauri-plugin-log's
// Webview target (every backend `log::` record arrives here); the browser/test build uses the noop.
// Kept behind a port so the Logs panel wiring stays unit-testable without a real webview.
