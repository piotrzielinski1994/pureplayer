import type { LogLine } from "@/lib/logging/log-line";

// The kv keys pureplayer's OWN backend emits (media.rs prepare_media / bg audio / remux + the
// FE log_playback bridge), so `field:value` search is useful against the local log lines.
export const KV_FIELDS = [
  "path",
  "container",
  "v",
  "a",
...
// `message:"audio or video stream"` stays ONE token; the quotes are stripped later by stripQuotes.
