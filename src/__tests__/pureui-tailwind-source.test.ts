...
// inside @pziel/pureui components (e.g. the scroll-area thumb's bg-foreground/20)
// are never generated. Without them the scroll-area thumb renders invisible.
// The fix is an explicit `@source` for the pureui bundle in src/index.css.
//
...
// surfaces the scroll-area thumb candidate. Delete the @source line and (1)
...
describe("app Tailwind build scans @pziel/pureui", () => {
  ...
  it("emits the pureui scroll-area thumb utility when scanned", () => {
    const scanner = new Scanner({
      sources: [{ base: PUREUI_DIST, pattern: "**/*", negated: false }],
    });
    const candidates = scanner.scan();

    // This scroll-area thumb utility lives ONLY inside pureui's scroll-area
    // component; if the app doesn't scan pureui it never gets generated.
